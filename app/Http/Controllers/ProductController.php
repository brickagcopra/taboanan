...
use Omnipay\Omnipay;
use taboanan\Payment;
use taboanan\Http\Requests;
use Paystack;
...
class ProductController extends Controller
{
	...
	$data = array('order' => $order, 'user_id' => $user_id);
	...
	public function purchase_full_details($token)
	{
	  ...
	  $data = array('purchase' => $purchase, 'user_id' => $user_id, 'billcountry' => $billcountry, 'shipcountry' => $shipcountry, 'product' => $product, 'refund_date' => $refund_date, 'today_date' => $today_date);
	...
	public function vendor_register(Request $request)
	{
	   ...
	   $company_goals = $request->input('company_goals');
	   ...
	   $user_id = $request->input('user_id');
	   $token = csrf_token();
	   $register_amount = $encrypter->decrypt($request->input('register_amount'));
	   ...
	   $check_register = Events::checkVendorReg($user_id,$event_token,$form_type);
	   if($check_register == 0)
	   {
	       if($registration_type == 'free')
		   {
			  ...
			  else
			  {
			   ...
			   $save_data = array('event_id' => $event_id, 'event_token' => $event_token, 'user_id' => $user_id, 'company_name' => $company_name, 'nature_business' => $nature_business, 'company_goals' => $company_goals, 'email_address' => $email_address, 'address_details' => $address_details, 'phone_number' => $phone_number, 'image_file' => $image_file, 'register_amount' => $register_amount, 'registration_type' => $registration_type, 'form_type' => $form_type, 'sponsors' => $sponsor_type, 'payment_type' => $payment_type, 'payment_date' => $payment_date, 'payment_status' => 'completed');
		       Events::saveVendorReg($save_data); 
			   return redirect()->back()->with('success', 'Your details has been registered. Thank You');
		   }
		   else
		   {
		       if ($request->hasFile('image_file')) 
			 ...
			 $save_data = array('event_id' => $event_id, 'event_token' => $event_token, 'user_id' => $user_id, 'company_name' => $company_name, 'nature_business' => $nature_business, 'company_goals' => $company_goals, 'email_address' => $email_address, 'address_details' => $address_details, 'phone_number' => $phone_number, 'image_file' => $image_file, 'register_amount' => $register_amount, 'registration_type' => $registration_type, 'form_type' => $form_type, 'sponsors' => $sponsor_type, 'payment_type' => $payment_type, 'payment_date' => $payment_date, 'payment_status' => $payment_status, 'purchase_token' => $purchase_token);
		     Events::saveVendorReg($save_data); 
			   ...
			   $record = array('register_amount' => $register_amount, 'purchase_token' => $purchase_token, 'payment_method' => $payment_method, 'form_type' => $form_type, 'paypal_url' => $paypal_url, 'paypal_email' => $paypal_email, 'site_currency' => $site_currency, 'website_url' => $website_url, 'stripe_mode' => $stripe_mode, 'stripe_publish_key' => $stripe_publish_key,  'token' => $token, 'encrypter' => $encrypter, 'package_type' => $package_type, 'package_amount' => $package_amount, 'registration_type' => $registration_type);
               return view('vendor-registration')->with($record);
	   ...
	   {
	     return redirect()->back()->with('error', 'Sorry You have already registered');
	   }
	...
	public function sponsor_register(Request $request)
	{
	   $allsettings = Settings::allSettings();
	   $encrypter = app('Illuminate\Contracts\Encryption\Encrypter');
	   $company_name = $request->input('company_name');
	   ...
	   $form_type = $request->input('form_type');
	   $sponsor_type = $request->input('sponsor_type');
	   if(!empty($request->input('package_amount')))
	   {
	   $packageamount = $request->input('package_amount');
	   $pieces = explode("-", $packageamount);
	   $package_amount = $pieces[0];
	   $package_type = $pieces[1];
	   }
	   else
	   {
	   $package_amount = 0;
	   ...
	   $paypal_email = $allsettings->paypal_email;
		...
		/* payment settings */	   
	   $check_register = Events::checkSponsorReg($user_id,$event_token,$form_type);
	   if($check_register == 0)
	   {
	       if($registration_type == 'free' && $sponsor_type == 'no')
		   {
		       if ($request->hasFile('image_file')) 
			   {
		   ...
		   elseif($registration_type == 'paid' && $sponsor_type == 'no')
		   {
		       if ($request->hasFile('image_file')) 
			   {
		      ...
		      $payment_status = 'pending';
			 $save_data = array('event_id' => $event_id, 'event_token' => $event_token, 'user_id' => $user_id, 'company_name' => $company_name, 'nature_business' => $nature_business, 'company_goals' => $company_goals, 'email_address' => $email_address, 'address_details' => $address_details, 'phone_number' => $phone_number, 'image_file' => $image_file, 'register_amount' => $register_amount, 'registration_type' => $registration_type, 'form_type' => $form_type, 'sponsors' => $sponsor_type, 'package_type' => $package_type, 'package_amount' => $package_amount, 'payment_type' => $payment_type, 'payment_date' => $payment_date, 'payment_status' => $payment_status, 'purchase_token' => $purchase_token);
		      Events::saveVendorReg($save_data); 
		   ...
		   elseif($registration_type == 'free' && $sponsor_type == 'yes')
		   {
		   ...
		   elseif($registration_type == 'paid' && $sponsor_type == 'yes')
		   {
	...
	public function confirm_vendor_register(Request $request)
	{
	   $encrypter = app('Illuminate\Contracts\Encryption\Encrypter');
	   $paypal_url = $request->input('paypal_url');
	   $paypal_email = $request->input('paypal_email');
	   ...
	   $paypal = '<form method="post" id="paypal_form" action="'.$paypal_url.'">
			  ...
			  <input type="hidden" value="'.$product_names.'" name="item_name">
			  <input type="hidden" value="'.$purchase_token.'" name="item_number">
			  <input type="hidden" value="'.$encrypter->decrypt($total_price).'" name="amount">
			  <input type="hidden" value="'.$site_currency.'" name="currency_code">
			  <input type="hidden" value="'.$return.'" name="return">
			  <input type="hidden" value="'.$cancel.'" name="cancel_return">
