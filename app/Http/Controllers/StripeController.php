...
class StripeController extends Controller
{
	...
	public function stripe_vendor_register(Request $request) 
	{
	   $encrypter = app('Illuminate\Contracts\Encryption\Encrypter');
	   $ord_token = $encrypter->decrypt($request->input('ord_token'));
	   $sid = 1;
	   $setting['setting'] = Settings::editGeneral($sid);
	   if($setting['setting']->stripe_mode == 0) 
		{
		...
		$check_update = array('payment_status' => 'completed', 'payment_id' => $payment_token, 'payment_type' => 'stripe');
