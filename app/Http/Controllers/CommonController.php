...
use taboanan\Models\Slideshow;
use taboanan\Models\Pages;
...
class CommonController extends Controller
{
	...
	$sid = 1;
	$setting['setting'] = Settings::editGeneral($sid);
	$event['view'] = Events::allHome();
	$data = array('setting' => $setting, 'event' => $event);
	return view('events')->with($data);
	 
	}
	
	public function view_events_single($slug)
	{
	$sid = 1;
	$setting['setting'] = Settings::editGeneral($sid);
	$single['event'] = Events::singlerEvent($slug);
	$recent['view'] = Events::recentEvent($slug);
	$encrypter = app('Illuminate\Contracts\Encryption\Encrypter');
	$data = array('setting' => $setting, 'single' => $single, 'recent' => $recent, 'encrypter' => $encrypter); 
	...
	public function view_user($slug)
	{
	   ...
	   $shop['product'] = Product::with('ProductImages')->where('product_status','=',1)->where('product_drop_status','=','no')->where('user_id','=',$user_id)->orderBy('product_id','desc')->get();
	...
	public function view_product($slug)
	{
	   ...
	   $another['product'] = Product::with('ProductImages')->where('product_status','=',1)->where('product_type','=',$product_type)->where('product_slug','!=',$slug)->where('product_drop_status','=','no')->take(4)->orderBy(DB::raw('RAND()'))->get();
